...
export default function BalanceBar() {
  ...
  const chipBase = `text-white font-bold rounded-xl no-underline whitespace-nowrap flex items-center gap-1 ${isNarrow ? 'text-[0.72rem] py-[0.2rem] px-[0.6rem]' : 'text-[0.8rem] py-1 px-3'}`;
      ...
      {(readyToEvolve || pending > 0) && (
        <div className="ml-auto flex gap-2 items-center">
          {readyToEvolve && (
            ...
            >
              <Zap size={13} /> {isNarrow ? '' : `${activeName} puede evolucionar`}
            </Link>
          )}
          {pending > 0 && (
            <Link
              ...
              className={`${chipBase} bg-warning`}
            >
              <PokeballIcon size={13} /> {isNarrow ? pending : `${pending} Pokémon por capturar`}
            </Link>
          )}
        </div>
      )}

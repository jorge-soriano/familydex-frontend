...
export default function ChildrenList() {
  ...
  const TH = (label: React.ReactNode, align: 'left' | 'center' | 'right' = 'left', w?: string) => (
    <th style={{ padding: '0.65rem 0.75rem', textAlign: align, width: w, fontSize: '0.72rem', fontWeight: 700, color: c.body, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: `2px solid ${c.stroke}`, background: c.primarySubtle, whiteSpace: 'nowrap' }}>
      {label}
    </th>
  );
        ...
        <div style={{ overflowX: 'auto', background: c.surface, borderRadius: 10, boxShadow: c.shadowSm }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 580 }}>
            ...
            <tbody>
              ...
              {children.map((child) => (
                <tr key={child.id} className="hover:bg-primary-subtle transition-colors duration-100" style={{ borderBottom: `1px solid ${c.stroke}`, opacity: child.isActive ? 1 : 0.55, background: child.isActive ? undefined : c.surface }}>
